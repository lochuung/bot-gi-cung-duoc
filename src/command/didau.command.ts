...
export class DidauCommand extends CommandMessage {
  ...
  async execute(args: string[], message: ChannelMessage, commandName?: string) {
    ...
    lines.push('   • `!dimoday` - Gợi ý ngẫu nhiên');
    lines.push('   • `!dimoday miền nam` - Lọc theo miền');
    lines.push('   • `!dimoday hà nội` - Lọc theo tỉnh/thành');
    lines.push('   • `!dimoday miền bắc hà nội` - Lọc kết hợp');
